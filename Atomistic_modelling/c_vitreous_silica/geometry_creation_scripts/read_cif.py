        ...
        for line in f:
            ...
            # If we are currently reading the atom sites... 
                ...
                if (len(cols) >= 4):
                    ...
                    print (cols[index_x], "#########QQQQQ")
                    k = float(cols[index_x])
                    print (k)
...
# Two atoms are on top of each other if they are less than "eps1" away.
#in angstrom
...
while (i < imax):
    ...
    for op in ops:
        ...
        new_atom = True
        for at in atoms:
            if (abs(at[1]-xn) < eps1  and  abs(at[2]-yn) < eps1  and  abs(at[3]-zn) < eps1):
                new_atom = False
...
if (bx < eps1  and  cx < eps1  and cy < eps1):
    make_rect_box = True
